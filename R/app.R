...
runGUI <- function() {
  server <- function(input, output, session) {
    ...
    output$saveGSAGGPLOT <- downloadHandler(
      ...
      content = function(file) {
        plt <- generateSobolBarGraph(gsaResultsDataframe = results$sobol$Results)
        saveRDS(object = plt, file = file)
      }
    )
    ...
    output$saveGSAPNG <- downloadHandler(
      ...
      content = function(file) {
        pltList <- generateSobolBarGraph(gsaResultsDataframe = results$sobol$Results)
    ...
    output$saveEFASTGGPLOT <- downloadHandler(
      ...
      content = function(file) {
        plt <- generateEFASTBarGraph(gsaResultsDataframe = results$efast$Results)
        saveRDS(object = plt, file = file)
      }
    )
    ...
    output$saveEFASTPNG <- downloadHandler(
      ...
      content = function(file) {
        pltList <- generateEFASTBarGraph(gsaResultsDataframe = results$efast$Results)
